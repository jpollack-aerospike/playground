...
int transaction::set (int fd, int64_t keyid, int64_t val) 
{
    recstate rs;
    rs.isRead = false;
    auto f = m_map.find (keyid);
    ...
    int rc = int_set (fd, this->id, keyid, val, rs.isRead ? rs.gen : 0, this->deadline);
    rs.isRead = false;
    m_map[keyid] = rs;
...
int transaction::get (int fd, int64_t keyid, int64_t& val) 
{
    recstate rs;
    rs.isRead = true;
    auto f = m_map.find (keyid);
    ...
    int rc = int_get (fd, this->id, keyid, val, &rs.gen, &rs.ver);
    if (rc) return rc;
